...
export const AppInfoComp: FC<{ siteInfo: AppInfo }> = ({ siteInfo }) => {
  ...
  return (
    <div>
      <div className='flex items-center py-2 text-xl font-medium text-gray-700 rounded-md justify-center'>Welcome to the New JESSI</div>
      ...
      <div className='text-center prompts-container'>
        {promptExamples.map((item, index) => (
          <span
            key={index}
            className='text-sm font-light text-gray-900 text-center pt-1 pb-2 my-1 px-3 mx-2 bg-gray-50 inline-block rounded-xl'
          >
            <b>{item}</b>
          </span>
        ))}
