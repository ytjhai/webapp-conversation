...
const Main: FC<IMainProps> = () => {
  ...
  */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [conversationData, appParams] = await Promise.all([fetchConversations(token), fetchAppParams(token)])
    ...
    sendChatMessage(data, {
      ...
      async onCompleted(hasError?: boolean) {
        ...
        if (getConversationIdChangeBecauseOfNew()) {
          const { data: allConversations }: any = await fetchConversations(token)
            ...
            hasSetInputs && (
              <div className='relative grow h-[200px] pc:w-[794px] max-w-full mobile:w-full pb-[66px] mx-auto mb-3.5 overflow-hidden'>
                <div className='h-full overflow-y-auto' ref={chatListDomRef}>
                  ...
                  />
                  <p className='text-center text-xs mt-4 text-gray-600 pl-4 absolute bottom-12 z-10 '>JESSI can make mistakes. Verify with Stylist Success.</p>
                </div>
              </div>)
          }
        </div>
